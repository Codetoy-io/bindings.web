...
function _setupEvents(container) {
    ...
    canvas.addEventListener("mousedown", e => {
        ...
        try {
            _wasmExports?.onMouseDown?.(e.button);
        }
        catch (e) {
            _onError(e);
        }
    ...
    canvas.addEventListener("mouseup", e => {
        ...
        try {
            _wasmExports?.onMouseUp?.(e.button);
        }
        catch (e) {
            _onError(e);
        }
    ...
    canvas.addEventListener("focus", () => {
        if (_focusOverlay)
            _focusOverlay.style.opacity = "0";
    });
    canvas.addEventListener("blur", () => {
        if (_focusOverlay)
            _focusOverlay.style.opacity = "1";
    });
    canvas.addEventListener("touchstart", e => {
        ...
        for (const t of e.changedTouches) {
            try {
                _wasmExports?.onTouchStart?.(t.identifier, t.clientX - r.left, t.clientY - r.top);
            }
            catch (e) {
                _onError(e);
            }
    ...
    canvas.addEventListener("touchmove", e => {
        ...
        for (const t of e.changedTouches) {
            ...
            try {
                _wasmExports?.onTouchMove?.(t.identifier, x, y);
            }
            catch (e) {
                _onError(e);
            }
    ...
    canvas.addEventListener("touchend", e => {
        ...
        for (const t of e.changedTouches) {
            ...
            try {
                _wasmExports?.onTouchEnd?.(t.identifier);
            }
            catch (e) {
                _onError(e);
            }
    ...
    new ResizeObserver(() => {
        ...
        try {
            _wasmExports?.resize?.(width, height);
        }
        catch (e) {
            _onError(e);
        }
